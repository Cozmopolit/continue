...
describe("MCPConnection", () => {
  ...
  describe("board gateway (board/pending)", () => {
    ...
    it("sends no params (server-resolved subscriptions), bounded by the board timeout", async () => {
      ...
      await conn.boardPending();

      expect(requestSpy).toHaveBeenCalledWith(
        { method: "board/pending", params: {} },
        expect.anything(),
  ...
  describe("board v2 (register)", () => {
    ...
    it("registers the handle via board/register, bounded by the register timeout", async () => {
      const requestSpy = vi
        .spyOn(Client.prototype, "request")
        .mockImplementation(async (req: any, resultSchema: any) => {
          if (req.method === "board/register") {
            return resultSchema.parse({ ok: true, handle: "citt-delta" });
          }
          throw new Error(`Unexpected request: ${req.method}`);
        });
      const conn = new MCPConnection(options);

      const res = await conn.boardRegister("citt-delta");

      expect(res).toEqual({ ok: true, handle: "citt-delta" });
      expect(requestSpy).toHaveBeenCalledWith(
