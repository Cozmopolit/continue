...
import { ChatMessage, MessagePart, Tool, ToolCall } from "../index.js";
import {
...
describe("countTokens (memoized, token-counting-hot-path.md)", () => {
  beforeEach(() => {
    countTokensCache.clear();
  });

  it("counts a simple string and is deterministic on cache hit", () => {
    const first = countTokens("Hello world!", "gpt-4");
    const second = countTokens("Hello world!", "gpt-4");
    expect(first).toBeGreaterThan(0);
    expect(second).toBe(first);
  });

  it("caches string content keyed by modelName|length|hash", () => {
    const result = countTokens("Hello world!", "gpt-4");
    expect(countTokensCache.size).toBe(1);
    const key = Array.from(countTokensCache.keys())[0];
    expect(key.startsWith("gpt-4|12|")).toBe(true);
    expect(countTokensCache.get(key)).toBe(result);

    // A cache hit must not add entries
    countTokens("Hello world!", "gpt-4");
    expect(countTokensCache.size).toBe(1);
  });

  it("keeps separate cache entries per model and applies multipliers", () => {
    const gpt = countTokens("Hello world!", "gpt-4");
    const claude = countTokens("Hello world!", "claude-3-opus");
    expect(countTokensCache.size).toBe(2);
    expect(claude).toBe(Math.ceil(gpt * 1.23));
  });
  ...
  it("counts MessagePart[] content without caching", () => {
    const asString = countTokens("Hello world!", "gpt-4");
    countTokensCache.clear();

    const asParts = countTokens(
      [{ type: "text", text: "Hello world!" }],
      "gpt-4",
    );
    expect(asParts).toBe(asString);
    expect(countTokensCache.size).toBe(0);

    const imageTokens = countTokens(
      [{ type: "imageUrl", imageUrl: "https://example.com/x.png" }],
      "gpt-4",
    );
    expect(imageTokens).toBe(1024);
    expect(countTokensCache.size).toBe(0);
  });

  it("treats undefined/null content defensively as empty (uncached)", () => {
  ...
  it("evicts the oldest entry when the cache is full", () => {
    for (let i = 0; i < COUNT_TOKENS_CACHE_LIMIT; i++) {
      countTokensCache.set(`dummy-${i}`, 0);
    }
    const result = countTokens("new content", "gpt-4");
    expect(result).toBeGreaterThan(0);
    expect(countTokensCache.size).toBe(COUNT_TOKENS_CACHE_LIMIT);
    expect(countTokensCache.has("dummy-0")).toBe(false);
    expect(countTokensCache.has("dummy-1")).toBe(true);
  });

  it("refreshes LRU recency on cache hit", () => {
    for (let i = 0; i < COUNT_TOKENS_CACHE_LIMIT - 2; i++) {
      countTokensCache.set(`dummy-${i}`, 0);
    }
    countTokens("keep me", "gpt-4");
    countTokens("second entry", "gpt-4");
    const keepKey = Array.from(countTokensCache.keys()).find((k) =>
      k.startsWith("gpt-4|7|"),
    );
    expect(keepKey).toBeDefined();

    // Hit refreshes recency: the key moves to the most recent position
    countTokens("keep me", "gpt-4");
    const keys = Array.from(countTokensCache.keys());
    expect(keys[keys.length - 1]).toBe(keepKey);
  });
});

describe("countToolsTokens (token-counting-hot-path.md)", () => {
  const tool: Tool = {
    type: "function",
    function: {
      name: "get_weather",
  ...
  it("returns the structural overhead for an empty tool list", () => {
    expect(countToolsTokens([], "gpt-4")).toBe(24);
  });

  it("counts tool schemas and is deterministic", () => {
    const first = countToolsTokens([tool], "gpt-4");
    const second = countToolsTokens([tool], "gpt-4");
    expect(first).toBeGreaterThan(24);
    expect(second).toBe(first);
  });

  it("applies the model multiplier (routed through countTokens)", () => {
    const gpt = countToolsTokens([tool], "gpt-4");
    const claude = countToolsTokens([tool], "claude-3-opus");
    expect(claude).toBeGreaterThan(gpt);
  });

  it("populates the shared countTokens cache", () => {
    countToolsTokens([tool], "gpt-4");
    expect(countTokensCache.size).toBeGreaterThan(0);
  });
});
